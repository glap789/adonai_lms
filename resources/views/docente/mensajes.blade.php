...
@section('content_header')
    <h1><b>Mensajería con Tutores</b></h1>
    <hr>
@stop
                                        ...
                                        <tbody>
                                            @foreach($mensajesRecibidos as $mensaje)
                                                ...
                                                <tr style="{{ !$leido ? 'font-weight: bold; background-color: #f0f8ff;' : '' }}">
                                                    ...
                                                    <td>
                                                        <a href="{{ route('docente.mensajeria.ver', $mensaje->id) }}" 
            ...
            <div class="modal-content">
                <form action="{{ route('docente.mensajeria.enviar') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header bg-primary">
                        <h5 class="modal-title"><i class="fas fa-envelope"></i> Nuevo Mensaje a Tutor</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <select name="estudiante_id" id="estudiante_id" class="form-control" required onchange="cargarTutores()">
                                <option value="">Seleccione un estudiante</option>
                                @foreach($tutores->unique('estudiante_id') as $item)
                                    <option value="{{ $item->estudiante_id }}">
                        ...
                        <div class="form-group">
                            <label>Tutor <b class="text-danger">*</b></label>
                            <select name="destinatario_user_id" id="tutor_id" class="form-control" required>
...
<script>
    ...
    function cargarTutores() {
        ...
        if (estudianteId && tutoresPorEstudiante[estudianteId]) {
            tutoresPorEstudiante[estudianteId].forEach(function(tutor) {
                const option = document.createElement('option');
                option.value = tutor.user_id;
                option.textContent = tutor.tutor_nombres + ' ' + tutor.tutor_apellidos + ' (' + tutor.relacion_familiar + ')';
